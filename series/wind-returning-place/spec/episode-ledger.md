...
## Episode 32 — 돌려 보는 한 장

- Canonical episode: `wind-returning-place` / `ep-032`, exactly 20 site-native-caption pages.
- Story motif: a matte cream photo backing with four soft corner clips, a round cobalt rotating plate, and one removable brass stop pin, `prop.rotating-print-back.001@v1`. Doyun first lays Yunseo's portrait pier photograph sideways, but she restores the direction she actually saw, chooses the vertical orientation, inserts the pin herself, and proves that the mounted photograph can be changed again. Soondeok supplies the distant-view test; Minjae remains absent in the present.
- Art/provenance: exactly 20 unique accepted text-free reader pages generated with built-in OpenAI imagegen and finalized by the shared series harness as 20 unique repository WebP assets. Twelve failed identity, composition, prop-evidence, or single-scene candidates were replaced before acceptance. Korean text is rendered only as site-native captions, with story, image, and lettering provenance recorded in `episode.json` and prompt addenda in `story/storyboard.yaml`.
- QA: story clarity is 20/20 concrete pages; continuity binds the three canonical characters and `prop.rotating-print-back.001@v1`; source and reader page counts are 20/20 with unique hashes. Repository verification, full `npm run check`, `git diff --check`, and isolated desktop 1440x1000 and mobile 390x844 browser checks pass with 20/20 loaded images, 20 captions, no failed images, no horizontal or figure overflow, no caption overflow, and zero console errors. Pages deployment and Live verification must still pass before publication is claimed.
- Next default: unified `ep-033`. Carry forward the owner's authority to revise how handed-over work is presented; avoid immediately repeating sideways pier posts, rotating photo backs, four corner clips, brass stop pins, portrait-versus-landscape tests, or owner-rotation payoffs.

## Episode 33 — 남겨 둔 가장자리

- Canonical episode: `wind-returning-place` / `ep-033`, exactly 20 site-native-caption pages.
- Story motif: a deep cream frame with a loose notched window mat, cobalt storage shelf, and four spacer pads, `prop.lift-off-photo-mat.001@v1`. Doyun's uniform crop hides the working hands and cobalt seaweed basket at the edge of Yunseo's photograph; Yunseo restores the full scene, chooses the uncropped view, and proves she can replace or remove the mat herself. Soondeok supplies the practical work-reading test; Minjae remains absent in the present.
- Art/provenance: exactly 20 unique accepted text-free reader pages generated with built-in OpenAI imagegen and finalized by the shared series harness as 20 unique repository WebP assets. Eleven failed identity, wardrobe, composition, prop, or scene-continuity candidates were replaced before acceptance. Korean text is rendered only as site-native captions, with story, image, and lettering provenance recorded in `episode.json` and accepted prompt addenda in `story/storyboard.yaml`.
- QA: story clarity is 20/20 concrete pages; continuity binds the three canonical characters and `prop.lift-off-photo-mat.001@v1`; source and reader page counts are 20/20 with unique hashes. Repository verification, full `npm run check`, and `git diff --check` pass. Isolated desktop 1440x1000 and mobile 390x844 browser checks pass with 20/20 loaded unique images, 20 captions, no failed images, no horizontal or figure overflow, no caption overflow, and zero console errors. Pages deployment and Live verification must still pass before publication is claimed.
- Next default: unified `ep-034`. Carry forward the owner's authority to revise the visible boundary of handed-over work; avoid immediately repeating cropped working hands, seaweed baskets, loose window mats, semicircular finger notches, cobalt storage shelves, hidden bicycle wheels, or owner-uncropping payoffs.

## Episode 34 — 고른 순서

- Canonical episode: `wind-returning-place` / `ep-034`, exactly 20 site-native-caption pages.
- Story motif: one pale wooden rail with exactly three separate sliding photo rests, `prop.reorderable-photo-rail.001@v1`. Doyun's tidy size order makes a harbor task read backward; Yunseo chooses rope, handcart, and empty pier as the actual work sequence, tests another order, and restores her own arrangement. Soondeok supplies the practical work-order test; a later Seoul owner chooses a family-photo order; Minjae remains absent in the present.
- Art/provenance: exactly 20 unique accepted text-free reader pages generated with built-in OpenAI imagegen and finalized by the shared series harness as 20 unique repository WebP assets. Four identity, count, or mechanism candidates on pages 3, 4, 15, and 16 were replaced before acceptance. Korean text is rendered only as site-native captions, with story, image, and lettering provenance recorded in `episode.json` and accepted prompt addenda in `story/storyboard.yaml`.
- QA: story clarity is 20/20 concrete pages; continuity binds the three canonical characters and `prop.reorderable-photo-rail.001@v1`; source and reader page counts are 20/20 with unique hashes. Repository verification, full `npm run check`, and `git diff --check` pass. Isolated desktop 1440x1000 and mobile 390x844 checks load 20/20 unique images with 20 captions, no failed images, no horizontal, figure, or caption overflow, and zero console errors. Pages deployment and Live verification must still pass before publication is claimed.
- Next default: unified `ep-035`. Carry forward the owner's authority to revise the order of handed-over work; avoid immediately repeating three-photo sequences, sliding wooden rails, movable photo rests, rope-handcart-empty-pier ordering, size-order mistakes, or owner-reordering payoffs.

## Episode 35 — 펴 둔 폭

- Canonical episode: `wind-returning-place` / `ep-035`, exactly 20 site-native-caption pages.
- Story motif: exactly two low matte cobalt oval paper weights with cream cork undersides, `prop.paired-panorama-edge-weights.001@v1`. Doyun compresses Yunseo's accordion harbor picture book to a narrow case width, hiding the small boat; Yunseo chooses a wider opening, positions both weights on the blank outer margins, tests a narrower setting, and restores her own width. Soondeok supplies the salt-stiffened-fold test; a later Seoul owner chooses the width of an accordion city map; Minjae remains absent in the present.
- Art/provenance: exactly 20 unique accepted text-free reader pages generated with built-in OpenAI imagegen and finalized by the shared series harness as 20 unique repository WebP assets. Eight identity, count, mechanism, composition, or setting candidates on pages 8, 10, 12, 13, 15, and 16 were replaced before acceptance. Korean text is rendered only as site-native captions, with story, image, and lettering provenance recorded in `episode.json` and accepted prompt addenda in `story/storyboard.yaml`.
- QA: story clarity is 20/20 concrete pages; continuity binds the three canonical characters and `prop.paired-panorama-edge-weights.001@v1`; source and reader page counts are 20/20 with unique hashes. Repository verification, full `npm run check`, and `git diff --check` pass. Isolated desktop 1440x1000 and mobile 390x844 checks load 20/20 unique images with 20 captions, no failed images, no horizontal, figure, or caption overflow, and zero console errors. Pages deployment and Live verification must still pass before publication is claimed.
- Next default: unified `ep-036`. Carry forward the owner's authority to revise how much of handed-over work is opened or exposed; avoid immediately repeating accordion books or maps, paired oval weights, cork undersides, lighthouse-to-boat panoramas, narrow glass cases, compressed folds, or owner-chosen-width payoffs.

## Episode 36 — 남겨 둔 울림

- Canonical episode: `wind-returning-place` / `ep-036`, exactly 20 site-native-caption pages.
- Story motif: exactly three independently removable muted cobalt felt sleeves, `prop.removable-felt-chime-dampers.001@v1`. Doyun's single transport tie silences all three brass strips; Yunseo chooses to free only the shorter center strip, tests a second moving strip, then restores her safer single-note setting herself. Soondeok supplies the strong-door-breeze test; a later Seoul owner chooses two moving pieces of a four-piece metal mobile; Minjae remains absent in the present.
- Art/provenance: exactly 20 unique accepted text-free reader pages generated with built-in OpenAI image generation and finalized by the shared series harness as 20 unique repository WebP assets. Six identity, wardrobe, mechanism, or count candidates on pages 4, 11, 12, 14, 16, and 17 were replaced before acceptance. Korean text is rendered only as site-native captions, with story, image, and lettering provenance recorded in `episode.json` and accepted prompt addenda in `story/storyboard.yaml`.
- QA: story clarity is 20/20 concrete pages; continuity binds the three canonical characters and `prop.removable-felt-chime-dampers.001@v1`; source and reader page counts are 20/20 with unique hashes. Repository verification, full `npm run check`, `git diff --check`, and isolated desktop 1440x1000 and mobile 390x844 checks pass with 20/20 loaded unique images, 20 captions, no failed images, no horizontal or figure overflow, and zero console errors. Pages deployment and Live verification must still pass before publication is claimed.
- Next default: unified `ep-037`. Carry forward the owner's authority to revise which parts of handed-over work remain free to move or sound; avoid immediately repeating wind mobiles, brass chime strips, cobalt felt sleeves, removable dampers, selecting audible pieces, strong-door-breeze tests, or owner-chosen sound payoffs.

## Episode 37 — 빌려 둔 둘

- Canonical episode: `wind-returning-place` / `ep-037`, exactly 20 site-native-caption pages.
- Story motif: one cream waxed-canvas roll with exactly four tool pockets and four fixed cobalt-and-cream pivot tabs, `prop.flip-share-tool-roll.001@v1`. Doyun assumes all four of Yunseo's brushes may be shared; Yunseo chooses only two, changes a returned wet brush to unavailable until it dries, and changes the stiff brush again when she needs it herself. Soondeok supplies the wet-versus-dry practical test; a later Seoul printmaker chooses one cleaned roller to share; Minjae remains absent in the present.
- Art/provenance: exactly 20 unique accepted text-free reader pages generated with built-in OpenAI image generation and finalized by the shared series harness as 20 unique repository WebP assets. Count or mechanism candidates on pages 9, 14, 19, and 20 were replaced before acceptance. Korean text is rendered only as site-native captions, with story, image, and lettering provenance recorded in `episode.json` and accepted prompt addenda in `story/storyboard.yaml`.
- QA: story clarity is 20/20 concrete pages; continuity binds the three canonical characters and `prop.flip-share-tool-roll.001@v1`; source and reader page counts, unique hashes, repository checks, isolated browser checks, Pages deployment, and Live verification must pass before publication is claimed.
- Next default: unified `ep-038`. Carry forward the owner's authority to revise which handed-over tools may be borrowed at different times; avoid immediately repeating brush rolls, four tool pockets, cobalt-and-cream flip tabs, wet returned brushes, borrowing permissions, or owner-selected shared-tool payoffs.

## Episode 38 — 바꿔 끼운 손잡이

- Canonical episode: `wind-returning-place` / `ep-038`, exactly 20 site-native-caption pages.
- Story motif: exactly two interchangeable same-length crank grips, `prop.interchangeable-crank-grips.001@v1`: one slim cream cork sleeve and one thick muted cobalt rubber sleeve. Doyun assumes the thick grip is universally easier, but Yunseo stops him from gluing it in place, chooses the slim grip for her bare hand, swaps to the thick grip for a bulky wet work glove, then restores the slim grip for the next dry sheet. Soondeok supplies the practical fit test; a later Seoul owner chooses the thick grip for her photo trimmer; Minjae remains absent in the present.
- Art/provenance: exactly 20 unique accepted text-free reader pages generated with built-in OpenAI image generation and finalized by the shared series harness as 20 unique repository WebP assets. Identity, mechanism, or count candidates on pages 8, 15, and 19 were replaced before acceptance. Korean text is rendered only as site-native captions, with story, image, and lettering provenance recorded in `episode.json` and accepted prompt addenda in `story/storyboard.yaml`.
- QA: story clarity is 20/20 concrete pages; continuity binds the three canonical characters and `prop.interchangeable-crank-grips.001@v1`; source and reader page counts are 20/20 with unique hashes. Repository verification, full `npm run check`, isolated browser checks, Pages deployment, and Live verification must pass before publication is claimed.
- Next default: unified `ep-039`. Carry forward the user's authority to choose and revise the physical fit of shared equipment; avoid immediately repeating paper presses or photo trimmers, paired grip sleeves, cork-versus-rubber thickness tests, threaded end caps, wet work gloves, or owner-chosen handle-fit payoffs.

## Episode 39 — 당겨 둔 작업판

- Canonical episode: `wind-returning-place` / `ep-039`, exactly 20 site-native-caption pages.
- Story motif: one low rectangular pale-wood work board with a centered semicircular finger notch, muted cobalt non-slip underside, and exactly four cream rubber feet, `prop.repositionable-work-board.001@v1`. Doyun fixes the board at the table center, but Yunseo removes that answer by pulling it within seated reach, pushing it farther away for a larger sheet while standing, and returning it again for the next card. Soondeok supplies the practical seated-versus-standing reach test; a later left-handed Seoul owner chooses a close left position; Minjae remains absent in the present.
- Art/provenance: exactly 20 unique accepted text-free reader pages generated with built-in OpenAI image generation and finalized by the shared series harness as 20 unique repository WebP assets. Duplicate-character, wardrobe, underside-evidence, board-count, or final-composition candidates on pages 6, 8, 11, 19, and 20 were replaced before acceptance. Korean text is rendered only as site-native captions, with story, image, and lettering provenance recorded in `episode.json` and accepted prompt addenda in `story/storyboard.yaml`.
- QA: story clarity is 20/20 concrete pages; continuity binds the three canonical characters and `prop.repositionable-work-board.001@v1`; source and reader page counts are 20/20 with unique hashes. Repository verification, full `npm run check`, isolated browser checks, Pages deployment, and Live verification must pass before publication is claimed.
- Next default: unified `ep-040`. Carry forward the user's authority to revise the working distance and placement of shared equipment; avoid immediately repeating movable work boards, centered semicircular finger notches, cobalt non-slip undersides, four cream rubber feet, seated-versus-standing reach tests, or left-handed board-placement payoffs.

## Episode 40 — 눕혀 둔 받침

- Canonical episode: `wind-returning-place` / `ep-040`, exactly 20 site-native-caption pages.
- Story motif: one low cream-painted drying rack with exactly five narrow cross slats and one separate muted cobalt triangular tilt wedge, `prop.removable-tilt-wedge-rack.001@v1`. Doyun's fixed early slope makes Yunseo's wet photograph slide and bend; Yunseo removes the wedge, keeps the photograph flat until its surface settles, then inserts the wedge herself so the remaining water drains. Soondeok supplies the practical sequence test; Doyun later uses the same flat-first, tilted-later order on his own Seoul print; Minjae remains absent in the present.
- Art/provenance: exactly 20 unique accepted text-free reader pages generated with built-in OpenAI image generation and finalized by the shared series harness as 20 unique repository WebP assets. Rack, character, wardrobe, setting, or action candidates on pages 1, 3, 4, 5, 8, 9, 10, 11, 13, 14, and 15 were replaced before acceptance. Korean text is rendered only as site-native captions, with story, image, and lettering provenance recorded in `episode.json` and accepted prompt addenda in `story/storyboard.yaml`.
- QA: story clarity is 20/20 concrete pages; continuity binds the three canonical characters and `prop.removable-tilt-wedge-rack.001@v1`; source and reader page counts, unique hashes, repository checks, isolated browser checks, Pages deployment, and Live verification must pass before publication is claimed.
- Next default: unified `ep-041`. Carry forward the user's authority to revise equipment settings as the work changes; avoid immediately repeating drying racks, five cross slats, triangular tilt wedges, wet photographs sliding downhill, flat-first drying tests, or a delayed-tilt Seoul payoff.

## Episode 41 — 옮겨 끼운 푸른 띠

- Canonical episode: `wind-returning-place` / `ep-041`, exactly 20 site-native-caption pages.
- Story motif: exactly two identical cream enamel pails and one removable cobalt rubber band with a tiny cream crescent repair stitch, `prop.movable-clean-water-band.001@v1`. Doyun contaminates clear water by relying on a fixed container identity; Yunseo empties, rinses, refills, tests, and moves the band herself when the pails' uses switch. Soondeok supplies the white-cloth water test; Doyun carries the changing-use marker into his Seoul print workroom; Minjae remains absent in the present.
- Art/provenance: exactly 20 unique accepted text-free reader pages generated with built-in OpenAI image generation and finalized by the shared series harness as 20 unique repository WebP assets. Setting, identity, wardrobe, band-count, or action candidates on pages 2, 3, 4, 12, 14, 16, and 17 were replaced before acceptance. Korean text is rendered only as site-native captions, with story, image, and lettering provenance recorded in `episode.json` and accepted prompt addenda in `story/storyboard.yaml`.
- QA: story clarity is 20/20 concrete pages; continuity binds the three canonical characters and `prop.movable-clean-water-band.001@v1`; source and reader page counts, unique hashes, repository checks, isolated browser checks, Pages deployment, and Live verification must pass before publication is claimed.
- Next default: unified `ep-042`. Carry forward the user's authority to move a practical marker when an object's current use changes; avoid immediately repeating paired enamel pails, clear-versus-cloudy water, cobalt rubber bands, cream crescent stitches, permanent color-label mistakes, or a marker-following-contents payoff.

## Episode 42 — 옮겨 잠근 수레

- Canonical episode: `wind-returning-place` / `ep-042`, exactly 20 site-native-caption pages.
- Story motif: one child-reachable pale-wood two-shelf supply cart with four gray casters and exactly two cobalt locking pedals, `prop.repositionable-supply-cart.001@v1`. Doyun's fixed floor outline forces Yunseo through a narrow route and bends her large paper, so she unlocks the cart, moves it beside the current task, walk-tests the passage, and locks it herself. Soondeok supplies the practical person-and-passage test; a later Seoul owner chooses positions for standing and seated work; Minjae remains absent in the present.
- Art/provenance: exactly 20 unique accepted text-free reader pages generated with built-in OpenAI image generation and finalized by the shared series harness as 20 repository WebP assets. Identity, setting, multi-panel composition, or cart-structure candidates on pages 5, 6, 8, and 16 were replaced before acceptance. Korean text is rendered only as site-native captions, with story, image, and lettering provenance recorded in `episode.json` and accepted prompt addenda in `story/storyboard.yaml`.
- QA: story clarity is 20/20 concrete pages; continuity binds the three canonical characters and `prop.repositionable-supply-cart.001@v1`; source and reader page counts, unique hashes, repository checks, isolated browser checks, Pages deployment, and Live verification must pass before publication is claimed.
- Next default: unified `ep-043`. Carry forward the user's authority to revise shared-equipment placement for the current worker and task; avoid immediately repeating rolling supply carts, exactly two shelves, cobalt caster pedals, floor tape rectangles, large paper caught at a doorway, path-walking tests, or owner-chosen cart-placement payoffs.
